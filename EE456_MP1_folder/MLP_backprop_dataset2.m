...
% load dataset 2
%load("Data\DataSet2_MP1.mat", "DataSet2");
%load("Data\DataSet2_MP1.mat", "DataSet2_targets");
...
% the code that is commented out below, is used to create the initial datasets, they are then loaded in afterwards

%rand_indices = randperm(6000, 4800); % choose 4800 (80% of 6000) random vectors
%S = DataSet2(rand_indices, :); % training set for dataset 1
%T = DataSet2_targets(rand_indices, :); % targets for dataset 1
%save("Data\Trainset_Dataset2.mat", "S");
%save("Data\Trainset_Dataset2_targets.mat", "T");

%remaining_indices = setdiff(1:6000, rand_indices); % the remaining 1200 (20% of 6000) vectors
%testset2 = DataSet2(remaining_indices, :); % testing set for dataset 1
%testset2_targets = DataSet2_targets(remaining_indices, :); % targets for the test set

%save("Data\Testset_Dataset2.mat", "testset2");
%save("Data\Testset_Dataset2_targets.mat", "testset2_targets");

load("Data\Trainset_Dataset2.mat", "S")
load("Data\Trainset_Dataset2_targets.mat", "T")
load("Data\Testset_Dataset2.mat", "testset2")
load("Data\Testset_Dataset2_targets.mat", "testset2_targets")
...
trainset_error = []; % vector for training error over time
testset_error = []; % vector for validation error over time
threshold = 0;

while iter_count < 20000000
    x = S(i,:); % random sample from the training set
    [x, z, z_in, y, y_in] = forward_train(x,W1,W2); % initial forward pass
    
    % update output weights
    j = 2; % this weight iteration starts at 2 as the first weight is for the bias
    ...
    j = 1;
    while j <= 20
        l = 2; % this iterator starts at 2 because 1 is the bias
        while l <= 3
            sigma_in = sigma * W2(j,:);
    ...
    % update the bias connections for the hidden layer
    j = 1;
    while j <= 20
        sigma_in = sigma * W2(j,:);
...
subplot(1,2,1);
plot(trainset_error, '-', 'LineWidth', 1, 'DisplayName', 'Training Error');
hold on;  % Hold the current plot so that the next plot is added to it
plot(testset_error, '-', 'LineWidth', 1, 'DisplayName', 'Validation Error');
