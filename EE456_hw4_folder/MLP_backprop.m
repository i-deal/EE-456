...
while iter_count < 50000000 %2
    ...
    % linearly anneal the learning rate to 10^(-5) by 8% per iteration
    if eta > 10^(-5)
        eta = eta/1.08;
    end
    i = randi([1, 4800]);
...
function [x, z, z_in, y, y_in] = forward_train(x,weight1,weight2)
    x = [1 x];
    z_in = weight1'*x';
    z = [1; tanh(z_in)];
    y_in = z'*weight2;
...
function y = forward_test(x,weight1,weight2)
